...
def get_feature_cols(mode: str = 'cpx_liq') -> List[str]:
    ...
    mode : str, default='cpx_liq'
        特征模式：'cpx_only', 'cpx_liq', 'cpx_oxide', 'liq_oxide', 'cpx_cation', 'all_oxide'
    ...
    if mode not in FEATURE_SETS:
        raise ValueError(f"未知特征模式: {mode}，支持 {list(FEATURE_SETS.keys())}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(data_path):
        ...
        print(f"文献来源数: {len(np.unique(data['groups']))}")
    else:

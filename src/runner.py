...
2. T 和 P 采用两条独立建模链路，分别训练、预测、校正与评估
3. 所有拟合操作只能在训练折内完成
...
class SingleTargetRunner:
    ...
    def _compute_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        ...
        返回包含 rmse, mae, r2, slope, intercept, bias_mean, resid_std 的字典
        """
...
class ExperimentRunner:
    ...
    T 和 P 采用两条独立建模链路，分别训练、预测、校正与评估
    """
    ...
    def run_experiment(self, X: np.ndarray, y_T: np.ndarray, y_P: np.ndarray,
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(X, y_T, groups)):
            ...
            preds_merged = preds_T.merge(
                preds_P[['row_id', 'P_true', 'P_pred_raw', 'P_pred_corr', 'P_residual']],
                on='row_id'
            )
            self.all_predictions.append(preds_merged)

            # 打印当前折指标
            print(f"  T: RMSE={metrics_T['T_rmse']:.2f}, R²={metrics_T['T_r2']:.4f}")
            print(f"  P: RMSE={metrics_P['P_rmse']:.3f}, R²={metrics_P['P_r2']:.4f}")
    ...
    def _save_final_results(self, results: Dict[str, Any]) -> None:
        ...
        print(f"\n{'='*60}")
        print("实验完成！汇总结果：")
        print(f"  T: RMSE={results['T_rmse_mean']:.2f}±{results['T_rmse_std']:.2f}, R²={results['T_r2_mean']:.4f}±{results['T_r2_std']:.4f}")
        print(f"  P: RMSE={results['P_rmse_mean']:.3f}±{results['P_rmse_std']:.3f}, R²={results['P_r2_mean']:.4f}±{results['P_r2_std']:.4f}")
...
def run_single_experiment(X: np.ndarray, y_T: np.ndarray, y_P: np.ndarray,
    ...
    Examples
    --------
    >>> config = ExperimentConfig(exp_name='exp1_baseline', model_type='catboost')

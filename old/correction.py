...
2. 禁止使用包含验证折的全局 OOF 拟合后校正验证折
3. 每个 outer fold 都要创建新的校正器实例
"""
...
class BiasCorrector(ABC):
    ...
    接口：
    - fit_on_oof(y_true, y_pred_oof): 用 OOF 预测拟合校正器
    ...
    def fit_on_oof(self, y_true: np.ndarray, y_pred_oof: np.ndarray) -> 'BiasCorrector':
        ...
        注意：确保 y_pred_oof 是通过 inner CV 生成的 OOF 预测，
              禁止使用 in-sample 训练集预测！
        """
        pass
...
class LinearBiasCorrector(BiasCorrector):
    ...
    通过线性回归 y_true ~ y_pred_oof 得到系数 a 和截距 b
    """

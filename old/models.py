...
class ExtraTreesWrapper(BaseThermoModel):
    ...
    **kwargs : dict
    ...
    def __init__(self, n_estimators: int = 200, max_depth: int = 10,
                 min_samples_split: int = 2, random_state: int = 42, n_jobs: int = -1, **kwargs):
    ...
    def set_params(self, **params) -> 'ExtraTreesWrapper':
        """设置模型参数（sklearn 兼容）"""
        for key, value in params.items():
            if key in ['n_estimators', 'max_depth', 'min_samples_split', 'random_state', 'n_jobs']:
                setattr(self, key, value)
            else:
                self.kwargs[key] = value
        return self
...
class XGBoostWrapper(BaseThermoModel):
    ...
    **kwargs : dict
    ...
    def __init__(self, n_estimators: int = 200, max_depth: int = 6,
                 learning_rate: float = 0.05, random_state: int = 42, n_jobs: int = -1, **kwargs):
        self.n_estimators = n_estimators
        self.max_depth = max_depth
        self.learning_rate = learning_rate
        self.random_state = random_state
    ...
    def set_params(self, **params) -> 'XGBoostWrapper':
        """设置模型参数（sklearn 兼容）"""
        for key, value in params.items():
            if key in ['n_estimators', 'max_depth', 'learning_rate', 'random_state', 'n_jobs']:
                setattr(self, key, value)
            else:
                self.kwargs[key] = value
        return self
...
class GroupAwareStacker(BaseThermoModel):
    ...
    4. predict() 时：用已训练的 base_models 对 X_val 预测，拼接成 Z_val
    ...
    def _model_signature(self) -> str:
        ...
        for model in self.base_models:
            ...
            base_sigs.append(f"{type(model).__name__}:{params}")
        meta_params = self.meta_model.get_params() if hasattr(self.meta_model, 'get_params') else {}
        meta_sig = f"{type(self.meta_model).__name__}:{meta_params}"
        return f"base={base_sigs}|meta={meta_sig}|inner_cv={self.inner_cv}|use_scaler={self.use_scaler}"

    def _compute_data_hash(self, X: np.ndarray, y: np.ndarray,
                           groups: Optional[np.ndarray] = None) -> str:
        """计算数据哈希用于缓存版本控制"""
        hasher = hashlib.md5()
        hasher.update(np.asarray(X).ravel().tobytes())
        hasher.update(np.asarray(y).ravel().tobytes())
        if groups is not None:
            group_bytes = "\n".join(map(str, np.asarray(groups).ravel())).encode("utf-8")
            hasher.update(group_bytes)
        hasher.update(self._model_signature().encode("utf-8"))
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, groups: Optional[np.ndarray] = None) -> 'GroupAwareStacker':
        ...
        """
        if groups is None:
            raise ValueError("GroupAwareStacker 需要 groups 参数进行训练")
        ...
        # 1. 生成 OOF 元特征
        data_hash = self._compute_data_hash(X, y, groups)
...
def get_model(name: str, **kwargs) -> BaseThermoModel:
    ...
    ----------
    name : str
        模型名称，支持 'catboost', 'extratrees', 'xgboost', 'stacking'
    **kwargs : dict
        模型参数
    ...
    if name == 'catboost':
        ...
        if 'base_models' not in kwargs:
            inner_cv = kwargs.pop('inner_cv', 5)
            cache_dir = kwargs.pop('cache_dir', None)
            use_heterogeneous = kwargs.pop('use_heterogeneous', True)
            ...
            if 'use_scaler' in kwargs:
                stacker.use_scaler = kwargs['use_scaler']
            if 'random_seed' in kwargs:
                stacker.random_seed = kwargs['random_seed']
            return stacker
...
def create_default_stacker(cache_dir: Optional[str] = None, inner_cv: int = 5,
                           use_heterogeneous: bool = True, n_jobs: int = -1) -> GroupAwareStacker:
    """
    创建默认配置的 Stacking 模型
    ...
    if use_heterogeneous:
        ...
        base_models = [
            ExtraTreesWrapper(n_estimators=200, max_depth=10, random_state=42, n_jobs=n_jobs),
            XGBoostWrapper(n_estimators=200, max_depth=6, learning_rate=0.05, random_state=42, n_jobs=n_jobs),
            CatBoostWrapper(iterations=200, depth=6, learning_rate=0.05, random_seed=42),
        ]
    else:
        # 同构基学习器：三个不同参数的 CatBoost
        base_models = [
